...
class Graph
  ...
  #takes a list of nodes and adds them one by one
  def add_nodes(nodes)
    nodes.each do |node|
       add_node(node)
     end
  ...
  #This adds nodes. I guess one by onoe
  def add_node(node)
    begin
  ...
  #This adds the edges. these are tuples of node 1-> node 2
...
%header
  ...
  GRAPH RESULT
%div
  %img{:src=>@result_png,:alt=>"graph image"}
